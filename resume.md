# Daniel Jones

Computer Scientist and Innovator

## Education:

Bitcoin Diploma, Chaincode 2025 BOSS Challenge, Btrust Builder Bitcoin-CLI Cohorts

Bachelor’s of Science in Computer Science and Art – Tennessee State University, 2015

Interactive Design and Game Development – Savannah College of Art and Design

## Technical Skills and Tools:
- Key Skills: Systems Engineering, Team and Project Management, Full Stack Development, Game Design, UI/UX, IT, Cybersecurity, Hardware Evaluation and Engineering
- Industry Expertise: Education, DoD, Fintech, Insurance, Healthcare
- Primary Languages/Frameworks: C#/.NET, Zig, C, C++, SQL, Javascript/Node/Bun, HTML, CSS
- Frontend Libraries: .NET Razor & Blazor, HTMX, Blaze/SpaceBars, React Native, Web Forms, Material Design, Bootstrap, jQueryUI
- Preferred IDEs: Visual Studio Code and neovim
- Cloud Computing: Azure, Digital Ocean, AWS
- Databases: SQL Server, MongoDB, PostgreSQL, SQLite, Oracle
- Source Control: git, SVN
- Game Development: Raylib, Unity, Unreal, Fusion Developer, XNA/Monogame
- Graphics: GIMP, Photoshop, Illustrator

## Work Experience:

### CodeCrew

October 2022 - Present

**Senior CodeSchool Instructor**

I train adult learners for Software Engineering and IT related lines work and lead our Instructors in curriculum development spanning introductory computer skills and more advanced Computer Science topics: Web/App Development, Machine Learning, Robotics, Embedded Systems, and Game Development. At CodeSchool, I also provide technical and career mentorship for students and Jr Instructors. Since standing up our in-house curriculum, I lead the team's yearly updates to our curriculum in Software Design, SDLC, Full Stack Development, API Development & Integration, Database Management, AI, and Cybersecurity Fundamentals.

### C. S. Lewis Marketing & dijital Technologies

December 2015 – Present

As the CTO for this marketing and technology firm, I oversee Technical and IT auditing and consulting projects that include Custom Application Development and Support, System Integrations, and Cybersecurity Consultations. Our current focus is on Bitcoin Education, Training, and Business Integration for custody and payment solutions in the Mid-South.

### Sedgwick

February 2019 - October 2022

**Senior IT Software Development Specialist**

Developed, maintained, tested and debugged software to meet generally defined requirements in a Windows client/server environment utilizing PL/SQL, Oracle, .Net, Microsoft SQL. Mentors team members and colleagues and regularly works as lead developer for projects.

- Contributed to UI development of XML report templates and frontend for .NET based medicare applications
- Served as the subject matter expert on the Jaspersoft report engine, parameters, claims management, intake services, carrier/client interfaces and vendor/business partner interfaces.
- Handled user needs as required in regards to application and data maintenance and system upgrades.
- Developed and modifies software to meet the specifications outlined in data processing requests, insurance carrier regulations.
- Tested and debugged assigned projects; assists in testing projects for other members of the programming staff. Investigates call escalations from the support staff; makes the necessary corrections to data and/or software.

### Global Medical Response

**Contract .NET Developer**

August 2021 - February 2022

- Optimized API performance for First Responders to log and pull Emergency data from a database instead of parsing server files.
- Migrated Oracle databases to SQL Server and utilized Entity Framework and LINQ in related .NET and SQL applications.

### First Horizon National Corporation 

September 2018 - February 2019

**ET Developer .NET**

Developed custom web applications and supported internal corporate applications. 
Translated the needs of customers and business partners into innovative, creative solutions. 
Responsible for direct collaboration with business analysts and other developers to deliver scalable, high quality & high performing solutions to support end user experience.

### Shelby County Schools 

June 2017 - September 2018

**IT Report Analyst, IT Database Advisor**

Supported the school system’s internal applications; maintained and updated application interfaces and data flows for school district employees. Regular tasks included updating stored procedures in SQL Server; managing shell scripts and creating command line tools to process file transfers; and documenting, updating, and maintaining application interface processes through VB Scripts, stored procedures, and server tasks. I also:
- Developed a (HTML/JS) programming curriculum for our summer 2017 interns.
- Cut estimated migration project timelines in half by scripting tools in C# to automate data migration from SQL Server to Oracle Cloud HCM
- Served as the Interface Lead on the district’s migration to the Oracle HCM and ERP systems, implementing the mapping, formatting, and planning the import/export of data from the legacy systems.

### System Improvements Inc.

July 2016 - December 2016

**Contract .NET Developer**

- Implemented a user dashboard with Javascript (jQuery) library Chart.JS for various reporting widgets.
- Regularly participated in bi-weekly Sprint Planning with daily Standups, code and bug Documentation, and usability Testing

### US Army TACOM

June 2014 - December 2015

**Contract Application Support Specialist**

- Supported the user-base of legacy .NET MVC apps while making upgrades to UI and web services connections to improve application usability and stay compliant with military standards.
- Worked with updating the client side of projects, including jQuery and Bootstrap plugins, while maintaining compatibility with IE9.
- .NET and Coldfusion Application Support
- Updated .NET Web Forms and .NET MVC applications to .NET 4.5
- Refactored the old web service based domain layer functionality to create a new and more organized internal and external API with Web API

### iD Tech Camps

Summers 2010 – 2013

**Camp Instructor @ Vanderbilt and Stanford**

Taught campers programming in Java, 3D Environment Design for with Unreal Development Kit, 2D Game Development in Multimedia Fusion and programming concepts in the visual tool, Scratch. Worked alongside staff members to coordinate shifts for activities and facilitated campers in a fun learning environment.

### Tennessee State University

2013

**IT Lab Technician** 
Maintained supplies, updated and repaired campus Mac, Linux, and Windows computer labs as a system admin. 

<link rel="stylesheet" href="./style.css"/>